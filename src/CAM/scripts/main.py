#!/usr/bin/env python3
"""
CAM Data Cleaner and Template Formatter

Transforms the CAM raw Excel (data_new.xlsx) to match the consolidated view template
...
- Provider name parsing and filtering
...
# Insurance type classification constants (copied from CIM)
INSURANCE_TYPE_MAP = [
...
class CAMTemplateFormatter:
    """Class to format CAM raw data to match consolidated view template."""
    ...
    def __init__(self, input_excel: str, output_excel: str):
        ...
        # Allowed providers for PROVIDER DATA
        self.allowed_providers = ["KHAMBATTA, SHEREZADE J", "MESIHA, NANCY", "EDWARDS, MATTHEW"]
    ...
    def _parse_emergency_contact_name(self, name: str) -> str:
        """Parse emergency contact name to First Last."""
        if not name:
            return ''
    ...
    def _extract_zip_from_address(self, address: str, city: str, state: str) -> Tuple[str, str]:
        """Extract ZIP from mailing address using city and state, and clean address."""
        if not address or not city or not state or str(address).lower().strip() in ('nan', 'none', ''):
            return '', ''
        address = str(address).strip()
        city = str(city).strip()
        state = str(state).strip()
        # Find zip after state
        pattern = rf'\b{re.escape(state)}\s+(\d{{5}})\b'
        match = re.search(pattern, address)
        if match:
            zip_code = match.group(1)
            # Remove city, state, zip from address
            to_remove = f"{city} {state} {zip_code}"
            street_address = address.replace(to_remove, '').strip()
            return street_address, zip_code
        return address, ''

    def _clean_phone(self, phone: str) -> Tuple[str, str]:
        """Clean phone: remove hm:, extract wk to work phone, remove invalid numbers."""
        if not phone or str(phone).lower().strip() in ('nan', 'none', ''):
            return '', ''
        phone = str(phone).strip()
        work_phone = ''
        # Extract work phone if present
        wk_match = re.search(r'wk[:\s]*([^\s]+)', phone, re.IGNORECASE)
        if wk_match:
            work_phone = wk_match.group(1).strip()
            phone = re.sub(r'wk[:\s]*[^\s]+', '', phone, flags=re.IGNORECASE).strip()
        # Remove hm: prefix
        phone = re.sub(r'hm[:\s]*', '', phone, flags=re.IGNORECASE).strip()
        # Remove invalid phone numbers
        if phone in ('0000000000', '9999999999', ''):
            phone = ''
        if work_phone in ('0000000000', '9999999999', ''):
            work_phone = ''
        return phone, work_phone

    def _clean_single_phone(self, phone: str) -> str:
        """Clean single phone number, remove invalid ones."""
        if pd.isna(phone) or not str(phone).strip() or str(phone).lower().strip() in ('nan', 'none', ''):
            return ''
        phone = str(phone).strip()
        if phone in ('0000000000', '9999999999', ''):
            return ''
    ...
    def _clean_string(self, value) -> str:
        """Clean string value, return empty if NaN or invalid."""
        if pd.isna(value):
            return ''
        return str(value).strip()

    def _clean_email(self, email: str) -> str:
        """Keep first email, lowercase."""
    ...
    def process_data(self):
        ...
        for _, row in df.iterrows():
            ...
            street_address, zip_code = self._extract_zip_from_address(mailing_address, city, state_val)
            # Remove ?? from street address
            street_address = street_address.replace('??', '').strip()

            # Clean phone and extract work phone
            phone_raw = str(row.get('Phone', ''))
            home_phone, work_phone = self._clean_phone(phone_raw)

            # Clean email
            email = self._clean_email(str(row.get('E-mail', '')))

            # Parse emergency contact name
            emerg_contact_name = self._parse_emergency_contact_name(str(row.get('Primary Emer Cont Name', '')))
            ...
            # Clean mobile phone
            mobile_phone = self._clean_single_phone(str(row.get('Mobile #', '')))

            # Language: default to English if empty
            language = str(row.get('Lang', '')).strip()
            if not language or language.lower() in ('nan', 'none', ''):
                language = 'English'

            # Provider data: from Encounter Provider
            encounter_provider = str(row.get('Encounter Provider', ''))
            provider_data = encounter_provider

            # Clinic facility: remove [number]
            clinic_facility = str(row.get('Dept/Loc', '')).strip()
            clinic_facility = re.sub(r'\s*\[.*?\]', '', clinic_facility).strip()

            # Provider name: normalized from provider_data
            provider_name = self._parse_provider_name(provider_data) if provider_data else ''

            # Emergency contact home phone
            emerg_home_phone = self._clean_single_phone(str(row.get('Primary Emer Cont Home Phone', '')))
            ...
            row_data = {
                ...
                'EMERGENCY CONTACT NAME': emerg_contact_name,
                'EMERGENCY RELATIONSHIP': self._clean_string(row.get('Primary Emer Cont Rel', '')),
                'EMERGENCY CONTACT HOME PHONE': emerg_home_phone,
                'EMERGENCY CONTACT MOBILE PHONE': '',
                'MEDICARE ID': self._clean_string(row.get('Medicare Sub ID', '')),
                'PRIMARY INSURANCE': self._clean_string(row.get('Primary Cvg', '')),
                'PRIMARY ID': self._clean_string(row.get('Primary Mem ID', '')),
                'PRIMARY GROUP': '',
                'SECONDARY INSURANCE': self._clean_string(row.get('Secondary Payer', '')),
                'SECONDARY ID': self._clean_string(row.get('Secondary Mem ID', '')),
                'SECONDARY GROUP': '',
                'TERITARY INSURANCE': self._clean_string(row.get('Tertiary Payer', '')),
                'TERITARY ID': self._clean_string(row.get('Tertiary Mem ID', '')),
                'TERITARY GROUP': '',
                'INSURANCE TYPE': classify_insurance(str(row.get('Primary Cvg', ''))),
                'CO-PAY': co_pay,
                **comorbidities,
                ...
                'NEXT APPT': self._parse_date(str(row.get('Next Appt Date', ''))),
                'PROVIDER DATA': provider_data,
                'PROVIDER NAME': provider_name,
                'CLINIC FACILITY': clinic_facility,
                'PRIMARY CARE PROVIDER': self._clean_string(row.get('PCP', '')),
                'MEDICATIONS': self._clean_string(row.get('Current Medications', '')),
                'ENCOUNTER NOTES': ''
            }
...
if __name__ == "__main__":
    input_file = Path(__file__).parent.parent / 'data_new.xlsx'
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = Path(__file__).parent.parent / 'output' / f'CAM_consolidated_{timestamp}.xlsx'
    output_file.parent.mkdir(exist_ok=True)

    formatter = CAMTemplateFormatter(str(input_file), str(output_file))
